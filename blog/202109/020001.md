...
void sort(RandomIt first, RandomIt last);  // 用运算符<排序范围[first, last)中的元素。不保证维持相等元素的顺序。

template<class RandomIt>
void stable_sort(RandomIt first, RandomIt last);  // 用运算符<排序范围[first, last)中的元素。保证维持相等元素的顺序。
```
...
(1)对于关联容器（set、map）、priority_queue、sort函数等有排序操作的情况，如果使用自定义的数据类型（通常是一个结构体），那么必须要重载<运算符（对于map是key为自定义数据类型时需要重载，因为map是根据key排序的）。
