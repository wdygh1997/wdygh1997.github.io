...
```
scanf(format, ...);  // 从标准输入流stdin读数据
// 返回成功赋值的参数数，若在赋值首个参数前输入失败返回EOF

sscanf(buffer, format, ...);  // 从字符串buffer读数据
// 返回成功赋值的参数数，若在赋值首个参数前输入失败返回EOF

printf(format, ...);  // 写结果到标准输出流stdout

sprintf(buffer, format, ...);  // 写结果到字符串buffer
```

格式字符串format表示如何读数据，在需要的位置插入格式符：
...
printf/sprintf常用宽度，精度格式符（以%d和%f为例，%lld和%lf同样适用）：
...
```
fgetc(stream);  // 从文件流stream（如标准输入流stdin）读下个字符
// 成功返回读到的字符，失败返回EOF

fgets(str, count, stream);  // 从文件流stream（如标准输入流stdin）读最多count-1个字符存储于str所指向的字符数组，若遇到文件尾或换行符则终止，后一情况下str将包含一个换行符
// 成功返回读到的字符串，失败返回空指针
```

### C++的输入输出（头文件\<iostream\>）
...
| double | IEEE双精度浮点数64位，1位符号位，11位阶码位（移码），52位尾数位（原码） | 约为($-2^{1024}$, $2^{1024}$) | 约为$-1.798 \times 10^{308}$ ~ $1.798 \times 10^{308}$ |
| char | 8位二进制数（1个字节） | [$-2^7$, $2^7-1$] | $-128$ ~ $127$ |

注：对于bool类型；表示方法没有定义，取决于编译器的实现；其他类型强制转换为bool时，0为false，非0为true。
...
// 例如定义结构体elem，重新定义大小关系先遵循属性a大小关系，再遵循属性b大小关系
struct elem {
    int a, b;
    bool operator < (const elem &e) const {  // 用于判定自身小于e是否为true
        if (a != e.a)
...
|:----|:---:|:---:|:---:|:---:|:---:|
|**说明**|定长数组|变长数组|双端队列|单向链表|双向链表|
|**头文件**|\<array\>|\<vector\>|\<deque\>|\<forward_list\>|\<list\>|
|**初始化&赋值**|= {元素序列}|= {元素序列}|= {元素序列}|= {元素序列}|= {元素序列}|
|**返回迭代器**||||||
|**容量**||||||
|**访问**||||||
|**修改**||||||
