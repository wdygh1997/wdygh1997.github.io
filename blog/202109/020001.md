...
```
一维数组mtx[len]赋值操作为fill(mtx, mtx+len, value)
二维数组mtx[len][len]赋值操作为fill(mtx[0], mtx[0]+len*len, value)
```
...
template<class ForwardIt, class T>
ForwardIt upper_bound(ForwardIt first, ForwardIt last, const T& value);  // 返回指向范围[first, last)中首个大于value的元素的迭代器，若找不到则返回last
```
